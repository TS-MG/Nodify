...
#include "znodifycontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNFY ought to be enough for anybody." - Bill Gates, 2017
    ui->zNFYpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNFYSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNFY->setEnabled(false);
        ui->pushButtonMintzNFY->setToolTip(tr("zNFY is currently disabled due to maintenance."));

        ui->pushButtonSpendzNFY->setEnabled(false);
        ui->pushButtonSpendzNFY->setToolTip(tr("zNFY is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNFYpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNFY_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNFY is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNFY...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNFY in ") +
...
void PrivacyDialog::on_pushButtonSpendzNFY_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNFY is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNFY
        sendzNFY();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNFY
    sendzNFY();
}

void PrivacyDialog::on_pushButtonZNodifyControl_clicked()
{
    ZNodifyControlDialog* zNodifyControl = new ZNodifyControlDialog(this);
    zNodifyControl->setModel(walletModel);
    zNodifyControl->exec();
}

void PrivacyDialog::setZNodifyControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNodifySelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNFY()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Nodify Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNFYpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNFY is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NFY </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNFY</b>";
    ...
    // use mints from zNodify selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNodifyControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNodifyControlDialog::GetSelectedMints();
    }
    ...
    // Clear znodify selector in case it was used
    ZNodifyControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNodify Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Nodify, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNodify Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNFY </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNFY "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNFY "));
    ui->labelzNFYAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNFYSyncStatus->setVisible(fShow);
}
