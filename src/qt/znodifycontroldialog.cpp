...
#include "ui_znodifycontroldialog.h"
...
std::list<std::string> ZNodifyControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNodifyControlDialog::listMints;

ZNodifyControlDialog::ZNodifyControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNodifyControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNodifyControlDialog::~ZNodifyControlDialog()
{
    delete ui;
}

void ZNodifyControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNodifyControlDialog::updateList()
{
...
void ZNodifyControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNodifyControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNodify_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNodifyControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNodifyControlDialog::GetSelectedMints()
{
...
void ZNodifyControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
