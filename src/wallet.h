...
// 13113 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_6666 = 13113;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NFY at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNFY send
enum ZerocoinSpendStatus {
    ZNFY_SPEND_OKAY = 0,                            // No error
    ZNFY_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNFY_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNFY_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNFY_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNFY_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNFY_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNFY_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNFY_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNFY_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNFY_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNFY_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNFY_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNFY_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNFY_SPENT_USED_ZNFY = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZNodifyAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 1500 * COIN) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 1500 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
